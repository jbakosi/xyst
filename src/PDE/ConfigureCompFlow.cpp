...
void
registerCompFlow( CGFactory& cf, std::set< ctr::PDEType >& cgt )
// *****************************************************************************
...
// *****************************************************************************
{
  ...
  const auto& gamma = g_inputdeck.get< tag::param, eq, tag::gamma >();
  if (gamma.size() > c)
    nfo.emplace_back( "ratio of specific heats", parameter( gamma[c][0] )) ;
