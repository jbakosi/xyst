...
// *****************************************************************************
{
  ...
  for (std::size_t e=0; e<inpoel.size()/4; ++e) {
    ...
    std::vector< const tk::real* > l( m_u.nprop() );
    for (ncomp_t c=0; c<m_u.nprop(); ++c) l[c] = m_lhs.cptr( c, 0 );
...
// *****************************************************************************
{
  if (g_inputdeck.get< tag::cmd, tag::benchmark >()) {
    ...
    auto nodefieldnames = numericFieldNames();
    // Collect field output from numerical solution requested by user
    auto nodefields = numericFieldOutput( m_u );
