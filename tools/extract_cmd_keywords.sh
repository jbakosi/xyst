...
export detail=$(for c in $commands; do ../Main/$exe -H $c ++quiet | grep -v '^Xyst>' | sed "s/$exe command-line keyword/@subsection ${exe}_cmd_kw_$c Keyword/" | sed 's/--/\\--/' | sed 's/Expected type:/_Expected type:_/' | sed 's/Lower bound:/_Lower bound:_/' | sed 's/Upper bound:/_Upper bound:_/' | sed 's/Expected valid choices:/_Expected valid choices:_/' | sed 's/</\\</g' | sed 's/>/\\>/g' ; done)
